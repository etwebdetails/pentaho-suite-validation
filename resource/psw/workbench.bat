...
rem if not exist %ROOT%\.schemaWorkbench mkdir %ROOT%\.schemaWorkbench
rem if not exist %ROOT%\.schemaWorkbench\log4j.xml copy log4j.xml %ROOT%\.schemaWorkbench
rem if not exist %ROOT%\.schemaWorkbench\mondrian.properties copy mondrian.properties %ROOT%\.schemaWorkbench

rem put mondrian.properties on the classpath for it to be picked up

rem set CP=.schemaWorkbench;%CP%
...
"%_PENTAHO_JAVA%" -Xms1024m -Xmx2048m -cp "%CP%" "-Dlog4j.configuration=log4j.xml" mondrian.gui.Workbench >> schemaworkbench.log 2>&1
